...
import (
	...
	gocache "github.com/admpub/go-cache"
...
func (l *Limiter) SetExpiring(bucketDefaultExpirationTTL, bucketExpireJobInterval time.Duration) *Limiter {
	...
	l.TokenBuckets.DefaultExpirationTTL = bucketDefaultExpirationTTL
	l.TokenBuckets.ExpireJobInterval = bucketExpireJobInterval

	// Default for ExpireJobInterval is every minute.
	if l.TokenBuckets.ExpireJobInterval <= 0 {
		l.TokenBuckets.ExpireJobInterval = time.Minute
	}

	if l.tokenBucketsWithTTL != nil {
		l.tokenBucketsWithTTL.Close()
	}

	l.tokenBucketsWithTTL = gocache.New(
		l.TokenBuckets.DefaultExpirationTTL,
		l.TokenBuckets.ExpireJobInterval,
	)
	return l
}
