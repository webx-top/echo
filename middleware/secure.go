	...
	return func(next echo.Handler) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if config.XSSProtection != "" {
				hdr.Set(echo.HeaderXXSSProtection, config.XSSProtection)
			}
			if config.ContentTypeNosniff != "" {
				hdr.Set(echo.HeaderXContentTypeOptions, config.ContentTypeNosniff)
			}
			if config.XFrameOptions != "" {
				hdr.Set(echo.HeaderXFrameOptions, config.XFrameOptions)
			}
			if (req.IsTLS() || (req.Header().Get(echo.HeaderXForwardedProto) == "https")) && config.HSTSMaxAge != 0 {
				...
				hdr.Set(echo.HeaderStrictTransportSecurity, fmt.Sprintf("max-age=%d%s", config.HSTSMaxAge, subdomains))
			}
			if config.ContentSecurityPolicy != "" {
				hdr.Set(echo.HeaderContentSecurityPolicy, config.ContentSecurityPolicy)
			}
			return next.Handle(c)
		}
	}
